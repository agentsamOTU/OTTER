...
void main() {
	...
	vec4 reflectStrength = texture(s_Reflect,inUV);

	vec3 result = (
		(u_AmbientCol * u_AmbientStrength) + // global ambient light
		(ambient + mix(diffuse,environment,reflectStrength.r) + specular) * attenuation // light factors from our single light
		) * inColor * mix(textureColor.rgb,environment,reflectStrength.r); // Object color
