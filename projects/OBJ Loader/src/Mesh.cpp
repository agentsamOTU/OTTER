			...
			while (std::getline(objToLoad, curLine))
			{
				if (curLine.at(0) == 'v')
				{
					...
					case ' ':
						...
						vertAsFloat.push_back(vertices.back().x);
						vertAsFloat.push_back(vertices.back().y);
						vertAsFloat.push_back(vertices.back().z);
						break;
					case 't':
		...
		for (int i = 0; i < line.size(); i++)
		{
			if (reading)
			{
				if (line[i] == '/')
				{
					...
					indexedVertices.push_back(unsigned int(std::stoi(vertHolder)-1));
					//std::cout << vertHolder;
					vertHolder = "";
				}
				else
